...
namespace EazyRentRevamp
{
    public class DataAccess
    {
        ...
        {
            // Using the Microsoft Access ODBC driver. Users should ensure the driver is installed.
            // Example: Driver={Microsoft Access Driver (*.mdb, *.accdb)};Dbq=C:\path\to\file.mdb;PWD=yourpassword;
            if (string.IsNullOrEmpty(_dbPath)) throw new InvalidOperationException("Database path is not set.");
            // Append the hardcoded password (replace later with secure retrieval)
            return $"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};Dbq={_dbPath};PWD={_dbPassword};";
        }
        ...
        private static string BuildConnectionString(string dbPath, bool includePassword)
        {
            ...
            return includePassword
                ? $"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};Dbq={dbPath};PWD={_dbPassword};"

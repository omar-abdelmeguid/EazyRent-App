        ...
        public void Save(Row sourceRow, string errorMessage)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    lock (_dbWriteLock)
                    {
                        _dataAccess.ExecuteOn(
                            ...
                            $"INSERT INTO [ErrorDBLog] " +
                            $"([Room_no],[Descr1],[Rent_no],[Date_OF_DB],[Amount],[Type]," +
                            $"[DebitAccount1],[DebitAccount2],[CreditAccount1],[CreditAccount2]," +
                            $"[DrcostCenter],[CrcostCenter],[Ser],[ERORR],[failed_requests_timestamp]) " +
                            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            ...
                            sourceRow.Values.TryGetValue("Date", out var dt)
                                ? dt ?? DBNull.Value
                                : DBNull.Value,

                            sourceRow.Values.TryGetValue("Amount", out var amt)
                                ? amt ?? DBNull.Value
                                : DBNull.Value,
                ...
                catch (System.Data.Odbc.OdbcException ex)
                {
                    ...
                    bool isTransient =
                        msg.Contains("could not lock file") ||
                        msg.Contains("file already in use") ||
                        msg.Contains("database has been placed in a state") ||
                        msg.Contains("sharing violation") ||
                        msg.Contains("locked") ||
                        msg.Contains("couldn't lock") ||
                        msg.Contains("duplicate values in the index");
